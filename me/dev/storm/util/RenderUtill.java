...
public class RenderUtill implements Util
{
    ...
    public static void gradientBox(final BlockPos pos, final Color color, final float lineWidth, final boolean outline, final boolean box, final int boxAlpha, final boolean speedmine) {
        final AxisAlignedBB bb = new AxisAlignedBB(pos.getX() - RenderUtill.mc.getRenderManager().viewerPosX, pos.getY() - RenderUtill.mc.getRenderManager().viewerPosY, pos.getZ() - RenderUtill.mc.getRenderManager().viewerPosZ, pos.getX() + 1 - RenderUtill.mc.getRenderManager().viewerPosX, pos.getY() + 1 - RenderUtill.mc.getRenderManager().viewerPosY, pos.getZ() + 1 - RenderUtill.mc.getRenderManager().viewerPosZ);
        RenderUtil.camera.setPosition(Objects.requireNonNull(RenderUtill.mc.getRenderViewEntity()).posX, RenderUtill.mc.getRenderViewEntity().posY, RenderUtill.mc.getRenderViewEntity().posZ);
        if (RenderUtil.camera.isBoundingBoxInFrustum(new AxisAlignedBB(bb.minX + RenderUtill.mc.getRenderManager().viewerPosX, bb.minY + RenderUtill.mc.getRenderManager().viewerPosY, bb.minZ + RenderUtill.mc.getRenderManager().viewerPosZ, bb.maxX + RenderUtill.mc.getRenderManager().viewerPosX, bb.maxY + RenderUtill.mc.getRenderManager().viewerPosY, bb.maxZ + RenderUtill.mc.getRenderManager().viewerPosZ))) {
            GlStateManager.pushMatrix();
            GlStateManager.enableBlend();
            GlStateManager.disableDepth();
            GlStateManager.tryBlendFuncSeparate(770, 771, 0, 1);
            ...
            final double percentage = RenderUtill.mc.playerController.curBlockDamageMP;
            float speedmineThing = Speedmine.getInstance().timer.getPassedTimeMs() / 1000.0f / (Speedmine.getInstance().breakTime * Storm.serverManager.getTpsFactor());
            ...
            if (outline) {
                drawBlockOutline(a, new Color(color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, 1.0f), (float)Speedmine.getInstance().lineWidth.getValue());
            }
            GL11.glDisable(2848);
            GlStateManager.depthMask(true);
            GlStateManager.enableDepth();
    ...
    public static void drawBlockOutline(final BlockPos pos, final Color color, final float linewidth, final boolean air) {
        ...
        if ((air || iblockstate.getMaterial() != Material.AIR) && RenderUtill.mc.world.getWorldBorder().contains(pos)) {
            assert RenderUtill.mc.renderViewEntity != null;
            final Vec3d interp = EntityUtil.interpolateEntity(RenderUtill.mc.renderViewEntity, RenderUtill.mc.getRenderPartialTicks());
            RenderUtil.drawBlockOutline(iblockstate.getSelectedBoundingBox((World)RenderUtill.mc.world, pos).grow(0.0020000000949949026).offset(-interp.x, -interp.y, -interp.z), color, linewidth);
        }
    }
    ...
    public static void drawOutlinedBlockESP(final BlockPos pos, final Color color, final float linewidth) {
        ...
        RenderUtil.drawBoundingBox(iblockstate.getSelectedBoundingBox((World)RenderUtill.mc.world, pos).grow(0.0020000000949949026).offset(-interp.x, -interp.y, -interp.z), linewidth, ColorUtil.toRGBA(color));
    }
    ...
    static {
        ...
        RenderUtill.itemRender = RenderUtill.mc.getItemRenderer().itemRenderer;
