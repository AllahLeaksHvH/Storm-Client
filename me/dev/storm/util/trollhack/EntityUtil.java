...
public class EntityUtil implements Util
{
    public static boolean isntValid(final EntityPlayer entity, final double range) {
        if (EntityUtil.mc.player.getDistance((Entity)entity) <= range && entity.getHealth() > 0.0f && entity != EntityUtil.mc.player) {
            final Storm instance = Storm.INSTANCE;
            if (!Storm.friendManager.isFriend(entity)) {
                return false;
            }
        }
        return true;
    ...
    public static float calculate(final double x, final double y, final double z, final EntityLivingBase base) {
        ...
        final double densityDistance = distance = (1.0 - distance) * EntityUtil.mc.world.getBlockDensity(new Vec3d(x, y, z), base.getEntityBoundingBox());
