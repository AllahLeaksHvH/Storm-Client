...
import java.awt.*;
import me.dev.storm.mixin.mixins.*;
import java.util.*;
import java.text.*;
import java.math.*;
...
public class EntityUtil2 implements Util
{
    ...
    public static void attackCrystals(final BlockPos pos, final boolean rotate) {
        final boolean sprint = EntityUtil2.mc.player.isSprinting();
        final int ping = Storm.serverManager.getPing();
        for (final EntityEnderCrystal crystal : EntityUtil2.mc.world.getEntitiesWithinAABB((Class)EntityEnderCrystal.class, new AxisAlignedBB(pos))) {
            if (EntityUtil2.attackTimer.passedMs((ping <= 50) ? 75L : 100L)) {
                if (rotate) {
                    final Vec3d crystalVec3d = crystal.getPositionVector();
                    RotationUtil.rotatePacket(crystalVec3d);
                }
                if (sprint) {
                    EntityUtil2.mc.player.connection.sendPacket((Packet)new CPacketEntityAction((Entity)EntityUtil2.mc.player, CPacketEntityAction.Action.STOP_SPRINTING));
                }
                EntityUtil2.mc.player.connection.sendPacket((Packet)new CPacketUseEntity((Entity)crystal));
                EntityUtil2.mc.player.connection.sendPacket((Packet)new CPacketAnimation(EnumHand.MAIN_HAND));
                if (sprint) {
                    EntityUtil2.mc.player.connection.sendPacket((Packet)new CPacketEntityAction((Entity)EntityUtil2.mc.player, CPacketEntityAction.Action.START_SPRINTING));
                }
                EntityUtil2.attackTimer.reset();
                break;
            }
        }
    }
    ...
    public static boolean isGhast(final Entity entity) {
        return entity instanceof EntityGhast || entity instanceof EntityGhast;
    }
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals((Object)EntityUtil2.mc.player) || (entity instanceof EntityPlayer && Storm.friendManager.isFriend(entity.getName())) || EntityUtil2.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer && colorFriends && Storm.friendManager.isFriend((EntityPlayer)entity)) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtil2.mc.world.playerEntities) {
            if (player.isInvisible() && player.getName().equals(EntityUtil2.mc.player.getName())) {
                continue;
            }
    ...
    public static List<BlockPos> getSphere(final BlockPos loc, final float r, final float h, final boolean hollow, final boolean sphere, final int plusY) {
