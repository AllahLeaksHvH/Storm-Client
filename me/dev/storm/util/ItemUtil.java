...
public class ItemUtil
{
    ...
    public static int getItemCount(final Item item) {
        int count = 0;
        for (int size = Util.mc.player.inventory.mainInventory.size(), i = 0; i < size; ++i) {
            final ItemStack itemStack = (ItemStack)Util.mc.player.inventory.mainInventory.get(i);
            if (itemStack.getItem() == item) {
                count += itemStack.getCount();
            }
        }
        final ItemStack offhandStack = Util.mc.player.getHeldItemOffhand();
