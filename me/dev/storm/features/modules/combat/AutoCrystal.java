...
public class AutoCrystal extends Module
{
    ...
    public void onPacketReceive(final PacketEvent.Receive event) {
        ...
        if (!this.justRender.getValue() && this.switchTimer.passedMs(this.switchCooldown.getValue()) && this.explode.getValue() && this.instant.getValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            final BlockPos pos;
            if (packet2.getType() == 51 && AutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + this.predictOffset.getValue() <= MathUtill.square(this.breakRange.getValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || (this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue())) || (this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue())))) {
                ...
                if (this.predictFriendDmg.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (final EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend != null && !AutoCrystal.mc.player.equals((Object)friend) && friend.getDistanceSq(pos) <= MathUtill.square(this.range.getValue() + this.placeRange.getValue()) && Storm.friendManager.isFriend(friend)) {
                            if (DamageUtill.calculateDamage(pos, (Entity)friend) <= EntityUtill.getHealth((Entity)friend) + 0.5) {
                                continue;
                            }
                            return;
                        }
                ...
                if (AutoCrystal.placedPos.contains(pos.down())) {
                    Label_0623: {
                        if (this.isRightThread() && this.superSafe.getValue()) {
                            ...
                            if ((selfDamage = DamageUtill.calculateDamage(pos, (Entity)AutoCrystal.mc.player)) - 0.5 <= EntityUtill.getHealth((Entity)AutoCrystal.mc.player)) {
                    ...
                    if (DamageUtill.canTakeDamage(this.suicide.getValue())) {
                        selfDamage = DamageUtill.calculateDamage(pos, (Entity)AutoCrystal.mc.player);
                    }
                    if (selfDamage + 0.5 < EntityUtill.getHealth((Entity)AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                        for (final EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                            if (player.getDistanceSq(pos) <= MathUtill.square(this.range.getValue()) && EntityUtill.isValid((Entity)player, this.range.getValue() + this.breakRange.getValue()) && (!this.antiNaked.getValue() || !DamageUtill.isNaked(player))) {
                                final float damage;
                                if ((damage = DamageUtill.calculateDamage(pos, (Entity)player)) <= selfDamage && (damage <= this.minDamage.getValue() || DamageUtill.canTakeDamage(this.suicide.getValue())) && damage <= EntityUtill.getHealth((Entity)player)) {
                                    continue;
                                }
            ...
            if (event.getPacket() instanceof SPacketSoundEffect && (packet6 = (SPacketSoundEffect)event.getPacket()).getCategory() == SoundCategory.BLOCKS && packet6.getSound() == SoundEvents.ENTITY_GENERIC_EXPLODE) {
                ...
                if (this.threadMode.getValue() == ThreadMode.SOUND && this.isRightThread() && AutoCrystal.mc.player != null && AutoCrystal.mc.player.getDistanceSq(pos) < MathUtill.square(this.soundPlayer.getValue())) {
    ...
    private boolean isRightThread() {
        return AutoCrystal.mc.isCallingFromMinecraftThread() || (!Storm.eventManager.ticksOngoing() && !this.threadOngoing.get());
    }
    ...
    public void onKeyInput(final InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoCrystal.mc.currentScreen instanceof StormGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchBack.getValue() && this.offhandSwitch.getValue() && this.offHand) {
                final Offhand module = Storm.moduleManager.getModuleByClass(Offhand.class);
                if (module.isOff()) {
    ...
    private void mapCrystals() {
            ...
            if (!entity.isDead && entity instanceof EntityEnderCrystal) {
                ...
                if (this.syncedFeetPlace.getValue() && entity.getPosition().down().equals((Object)this.syncedCrystalPos) && this.damageSync.getValue() != DamageSync.NONE) {
                    ...
                    if (selfDamage + 0.5 < EntityUtill.getHealth((Entity)AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                        ...
                        for (final EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                            if (player.getDistanceSq(entity) > MathUtill.square(this.range.getValue())) {
                                continue;
                            }
                            if (EntityUtill.isValid((Entity)player, this.range.getValue() + this.breakRange.getValue())) {
                                ...
                                if ((this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL) || !Storm.friendManager.isFriend(player.getName())) {
                                    continue;
                                }
                                final float damage;
                                if ((damage = DamageUtill.calculateDamage(entity, (Entity)player)) <= EntityUtill.getHealth((Entity)player) + 0.5) {
                                    continue;
                                }
        ...
        if (this.webAttack.getValue() && this.webPos != null) {
            if (AutoCrystal.mc.player.getDistanceSq(this.webPos.up()) > MathUtill.square(this.breakRange.getValue())) {
                this.webPos = null;
            }
            else {
        ...
        if (this.shouldSlowBreak(true) && maxDamage < this.minDamage.getValue() && (AutoCrystal.target == null || EntityUtill.getHealth((Entity)AutoCrystal.target) > this.facePlace.getValue() || (!this.breakTimer.passedMs(this.facePlaceSpeed.getValue()) && this.slowFaceBreak.getValue() && Mouse.isButtonDown(0) && this.holdFacePlace.getValue() && this.holdFaceBreak.getValue()))) {
            this.efficientTarget = null;
            return;
        }
        if (this.packets.getValue() == 1) {
    ...
    private void placeCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue()) && this.place.getValue() && (this.offHand || this.mainHand || this.switchMode.getValue() == Switch.CALC || (this.switchMode.getValue() == Switch.BREAKSLOT && this.switching))) {
            ...
            if (AutoCrystal.target != null && this.placePos != null) {
                if (!this.offHand && !this.mainHand && this.autoSwitch.getValue() != AutoSwitch.NONE && (this.currentDamage > this.minDamage.getValue() || (this.lethalSwitch.getValue() && EntityUtill.getHealth((Entity)AutoCrystal.target) <= this.facePlace.getValue())) && !this.switchItem()) {
                    return;
                }
                if (this.currentDamage < this.minDamage.getValue() && this.limitFacePlace.getValue()) {
                    crystalLimit = 1;
                }
                if (this.currentDamage >= this.minMinDmg.getValue() && (this.offHand || this.mainHand || this.autoSwitch.getValue() != AutoSwitch.NONE) && (this.crystalCount < crystalLimit || (this.antiSurround.getValue() && this.lastPos != null && this.lastPos.equals((Object)this.placePos))) && (this.currentDamage > this.minDamage.getValue() || this.minDmgCount < crystalLimit) && this.currentDamage >= 1.0 && (DamageUtill.isArmorLow(AutoCrystal.target, this.minArmor.getValue()) || EntityUtill.getHealth((Entity)AutoCrystal.target) <= this.facePlace.getValue() || this.currentDamage > this.minDamage.getValue() || this.shouldHoldFacePlace())) {
    ...
    private void calculateDamage(final EntityPlayer targettedPlayer) {
        ...
        for (final BlockPos pos : BlockUtill.possiblePlacePositions(this.placeRange.getValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            if (!BlockUtill.rayTracePlaceCheck(pos, (this.raytrace.getValue() == Raytrace.PLACE || this.raytrace.getValue() == Raytrace.FULL) && AutoCrystal.mc.player.getDistanceSq(pos) > MathUtill.square(this.placetrace.getValue()), 1.0f)) {
                continue;
            }
            float selfDamage = -1.0f;
            if (DamageUtill.canTakeDamage(this.suicide.getValue())) {
            ...
            if (selfDamage + 0.5 >= EntityUtill.getHealth((Entity)AutoCrystal.mc.player)) {
                continue;
            }
            ...
            if (targettedPlayer != null) {
                final float playerDamage = DamageUtill.calculateDamage(pos, (Entity)targettedPlayer);
                if (this.calcEvenIfNoDamage.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (final EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend != null && !AutoCrystal.mc.player.equals((Object)friend) && friend.getDistanceSq(pos) <= MathUtill.square(this.range.getValue() + this.placeRange.getValue()) && Storm.friendManager.isFriend(friend)) {
                            ...
                            if ((friendDamage = DamageUtill.calculateDamage(pos, (Entity)friend)) <= EntityUtill.getHealth((Entity)friend) + 0.5) {
                                continue;
                            }
                            friendPop = true;
                            break;
                ...
                if (this.isDoublePoppable(targettedPlayer, playerDamage) && (currentPos == null || targettedPlayer.getDistanceSq(pos) < targettedPlayer.getDistanceSq(currentPos))) {
                    ...
                    if (playerDamage <= selfDamage && (playerDamage <= this.minDamage.getValue() || DamageUtill.canTakeDamage(this.suicide.getValue())) && playerDamage <= EntityUtill.getHealth((Entity)targettedPlayer)) {
                        continue;
                    }
                    maxDamage = playerDamage;
                    currentTarget = targettedPlayer;
                ...
                for (final EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    if (EntityUtill.isValid((Entity)player, this.placeRange.getValue() + this.range.getValue())) {
                        if (this.antiNaked.getValue() && DamageUtill.isNaked(player)) {
                            continue;
                        }
                        final float playerDamage2 = DamageUtill.calculateDamage(pos, (Entity)player);
                        if (this.doublePopOnDamage.getValue() && this.isDoublePoppable(player, playerDamage2) && (currentPos == null || player.getDistanceSq(pos) < player.getDistanceSq(currentPos))) {
                            ...
                            if (playerDamage2 <= selfDamage && (playerDamage2 <= this.minDamage.getValue() || DamageUtill.canTakeDamage(this.suicide.getValue())) && playerDamage2 <= EntityUtill.getHealth((Entity)player)) {
                                continue;
                            }
                            maxDamage = playerDamage2;
                            currentTarget = player;
                        ...
                        if ((this.antiFriendPop.getValue() != AntiFriendPop.ALL && this.antiFriendPop.getValue() != AntiFriendPop.PLACE) || player == null || player.getDistanceSq(pos) > MathUtill.square(this.range.getValue() + this.placeRange.getValue()) || !Storm.friendManager.isFriend(player)) {
                            continue;
                        }
                        final float friendDamage2;
                        if ((friendDamage2 = DamageUtill.calculateDamage(pos, (Entity)player)) <= EntityUtill.getHealth((Entity)player) + 0.5) {
                            continue;
                        }
    ...
    private EntityPlayer getTarget(final boolean unsafe) {
        ...
        EntityPlayer currentTarget = null;
        for (final EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
            if (!EntityUtill.isntValid((Entity)player, this.placeRange.getValue() + this.range.getValue()) && (!this.antiNaked.getValue() || !DamageUtill.isNaked(player))) {
                if (unsafe && EntityUtill.isSafe((Entity)player)) {
                    continue;
                }
        ...
        if (this.predictPos.getValue() && currentTarget != null) {
            ...
            newTarget.setHealth(EntityUtill.getHealth((Entity)currentTarget));
    ...
    private void rotateTo(final Entity entity) {
        switch (this.rotate.getValue()) {
            ...
            case BREAK:
            case ALL: {
                final float[] angle = MathUtill.calcAngle(AutoCrystal.mc.player.getPositionEyes(AutoCrystal.mc.getRenderPartialTicks()), entity.getPositionVector());
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    Storm.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
                this.yaw = angle[0];
                this.pitch = angle[1];
    ...
    private void rotateToPos(final BlockPos pos) {
        switch (this.rotate.getValue()) {
            ...
            case PLACE:
            case ALL: {
                final float[] angle = MathUtill.calcAngle(AutoCrystal.mc.player.getPositionEyes(AutoCrystal.mc.getRenderPartialTicks()), new Vec3d((double)(pos.getX() + 0.5f), (double)(pos.getY() - 0.5f), (double)(pos.getZ() + 0.5f)));
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    Storm.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
    ...
    private boolean isDoublePoppable(final EntityPlayer player, final float damage) {
        final float health;
        if (this.doublePop.getValue() && (health = EntityUtill.getHealth((Entity)player)) <= this.popHealth.getValue() && damage > health + 0.5 && damage <= this.popDamage.getValue()) {
    ...
    private boolean isValid(final Entity entity) {
        return entity != null && AutoCrystal.mc.player.getDistanceSq(entity) <= MathUtill.square(this.breakRange.getValue()) && (this.raytrace.getValue() == Raytrace.NONE || this.raytrace.getValue() == Raytrace.PLACE || AutoCrystal.mc.player.canEntityBeSeen(entity) || (!AutoCrystal.mc.player.canEntityBeSeen(entity) && AutoCrystal.mc.player.getDistanceSq(entity) <= MathUtill.square(this.breaktrace.getValue())));
    }
        ...
        public void run() {
            if (this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.autoCrystal.isOn() && this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                    while (Storm.eventManager.ticksOngoing()) {}
