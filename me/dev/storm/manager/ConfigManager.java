...
public class ConfigManager implements Util
{
    ...
    public static void setValueFromJson(final Feature feature, final Setting setting, final JsonElement element) {
        ...
        switch (type) {
            ...
            default: {
                Storm.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }
    ...
    private static void loadFile(final JsonObject input, final Feature feature) {
        for (final Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Storm.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                }
                catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public void loadConfig(final String name) {
        final List<File> files = Arrays.stream((Object[])Objects.requireNonNull((T[])new File("storm").listFiles())).filter(File::isDirectory).collect((Collector<? super Object, ?, List<File>>)Collectors.toList());
        if (files.contains(new File("storm/" + name + "/"))) {
            this.config = "storm/" + name + "/";
        }
        else {
            this.config = "storm/config/";
        }
    ...
    public boolean configExists(final String name) {
        final List<File> files = Arrays.stream((Object[])Objects.requireNonNull((T[])new File("storm").listFiles())).filter(File::isDirectory).collect((Collector<? super Object, ?, List<File>>)Collectors.toList());
        return files.contains(new File("storm/" + name + "/"));
    }
    ...
    public void saveConfig(final String name) {
        ...
        Storm.friendManager.saveFriends();
        for (final Feature feature : this.features) {
            try {
    ...
    public void init() {
        this.features.addAll((Collection<? extends Feature>)Storm.moduleManager.modules);
        this.features.add(Storm.friendManager);
        final String name = this.loadCurrentConfig();
        this.loadConfig(name);
        Storm.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(final Path path, final Feature feature) throws IOException {
        ...
        try {
            ...
            Storm.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
